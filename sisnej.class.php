...
class Sisnej {
    ...
    function DarSeguimiento($id,$tipo){
        date_default_timezone_set('America/Chicago'); 
        $con = new DBManager(); //creamos el objeto $con a partir de la clase DBManager
        $dbh = $con->conectar("mysql"); //Pasamos como parametro que la base de datos a utilizar para el caso MySQL.
        $sql = "INSERT INTO seguimiento_notificacion VALUES(:id,:Fecha,:Tipo)";
        $query = $dbh->prepare($sql); // Preparamos la consulta para dejarla lista para su ejecucion
        $query->bindParam(":id",$id);
    ...
    function consultar_notificaciones($juzgado) {
        ...
        INNER JOIN municipios as m ON j.IdMunic=m.IdMunic and j.IdDepto=m.IdDepto INNER JOIN departamentos AS d ON m.IdDepto=d.IdDepto where n.idNotificador=:juzgado ORDER BY FechaEnvio DESC";
        $query = $dbh->prepare($sql); // Preparamos la consulta para dejarla lista para su ejecucion
     ...
     function consultar_seguimiento($id) {
        $con = new DBManager(); //creamos el objeto $con a partir de la clase DBManager
        $dbh = $con->conectar("mysql"); //Pasamos como parametro que la base de datos a utilizar para el caso MySQL.
        $sql = "SELECT * FROM seguimiento_notificacion WHERE idNotificacion=:id";
        $query = $dbh->prepare($sql); // Preparamos la consulta para dejarla lista para su ejecucion
        $query->bindParam(":id",$id);
        $query->execute(); // Ejecutamos la consulta
        if ($query)
  ...
  PRIMARY KEY (`id`)
	...
	function crear_cuenta($campos) {
		...
		$add->bindParam(":Munic",$campos[8]);
		$add->bindParam(":Depto",$campos[9]);
